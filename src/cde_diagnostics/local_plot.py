...
def local_plot(x_train, pit_train, x_test, alphas=np.linspace(0.0, 0.999, 101), plot_type='ALP', clf_name='MLP',
                  confidence_bands=False, conf_alpha=0.05, n_bins=7, n_trials=100, figsize=(5,4)):
    ...
    if confidence_bands:
        ...
        for k in range(n_trials):
            ...
            H, bins = np.histogram(pd.Series(all_rhat_alphas_k), bins=np.linspace(0,1,n_bins+1))
            all_H += [H]
        ...
        if plot_type == 'ALP':
            ...
            low_H = np.quantile(np.array(all_H), q=conf_alpha/2, axis=0)
            upp_H = np.quantile(np.array(all_H), q=1-conf_alpha/2, axis=0)
            plt.plot(np.linspace(0,1,n_bins+1), np.insert(low_H, 0, low_H[0]), '-', drawstyle='steps', color='grey')
            plt.plot(np.linspace(0,1,n_bins+1), np.insert(upp_H, 0, upp_H[0]), '-', drawstyle='steps', color='grey')
            plt.fill_between(x=np.linspace(0,1,n_bins+1),
                             y1=np.insert(low_H, 0, low_H[0]),
                             y2=np.insert(upp_H, 0, upp_H[0]),
                             step="pre",
